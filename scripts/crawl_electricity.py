...
def crawl_country(country_name, country_en):
    """爬取单个国家的电价（家庭用户）"""
    url = f'https://www.globalpetrolprices.com/{country_en}/electricity_prices/'
    headers = {
    ...
    try:
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if len(cells) >= 2:
                    ...
                    if 'Households' in text or 'households' in text:
        ...
        for i, line in enumerate(lines):
            if 'electricity_prices' in line.lower() or 'electricity prices' in line.lower():
                for j in range(i+1, min(i+5, len(lines))):
                    try:
                        price = float(lines[j].strip())
                        if 0.01 < price < 1000:  # 合理价格范围
                            return price
                    except ValueError:
                        continue
    except Exception as e:
        print(f"爬取{country_name}电价失败: {e}")
    return None
...
def crawl_all():
    ...
    for country_name, config in COUNTRIES.items():
        ...
        if price is not None:
            # 转换为人民币
            cny_price = round(price * rates[config['currency']], 3)
            results.append({
    ...
    # 保存数据文件
    data_dir = os.path.join(os.path.dirname(__file__), '..', 'data', 'electricity')
